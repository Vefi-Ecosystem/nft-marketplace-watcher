...
import { handleCollectionDeploymentEvent } from './handlers';
...
export function handleEvents(url: string, address: string, network: string) {
  ...
  provider.on({ address, topics: [mintHash] }, console.log);
  provider.on({ address, topics: [marketItemCreatedHash] }, console.log);
  provider.on({ address, topics: [marketItemCancelledHash] }, console.log);
  provider.on({ address, topics: [saleMadeHash] }, console.log);
